...
const SelectWithDetail = ({ label, value, onChange }: { label: string, value: string, onChange: (val: string) => void }) => {
  // Determine initial state based on value string
...
const DropdownAndText = ({ label, value, options, onChange }: { label: string, value: string, options: string[], onChange: (val: string) => void }) => {
  // Find which option the value starts with
  ...
  const handleTextChange = (newDetail: string) => {
    if (selectedOption) {
      ...
      // If no option selected, essentially just text? Or stick to requiring option?
      // Let's assume if no option, we just change the detail? 
      // But we need an option. If empty, maybe just text?
      // For safety, if no option selected, maybe select the first one? Or just keep as text?
      // Let's just keep as text if nothing matches, or maybe force user to pick?
      // Better: if no option, treat as just text? No, user wants dropdown + text.
      // If no option selected, we can't really form the "Option: Detail" structure well.
      // Let's just pass text if no option.
      onChange(newDetail);
    }
  };
...
const FaceSheet: React.FC<FaceSheetProps> = ({ data, onUpdate }) => {
  ...
  const handleFamilyUpdate = (index: number, field: keyof FamilyMember, value: string) => {
  ...
  return (
    <div className="face-sheet-container bg-white shadow-xl mx-auto max-w-[210mm] print:shadow-none print:mx-0">
      ...
      {/* PAGE 1: 厳密なA4高さとパディングを設定 (A4高さ=297mm) */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-4 h-[240px]">
              <div className="flex flex-col h-full">
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[10px] font-bold text-indigo-700 block truncate">ジェノグラム（家系図）ドロップ可</label>
                ...
                <div
                  className={`border-2 border-dashed rounded-xl flex-1 min-h-0 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-200 cursor-pointer group
                    ${dragTarget === 'genogram' ? 'border-indigo-500 bg-indigo-100 scale-[1.02] shadow-lg' : 'border-indigo-200 bg-indigo-50/50 hover:bg-indigo-50'}
                  `}
                ...
                >
                  {data.genogramUrl ? (
                    <>
                      <img src={data.genogramUrl} className="absolute inset-0 w-full h-full object-contain p-2" alt="Genogram" />
                      <button
                        onClick={(e) => handleRemoveImage('genogramUrl', e)}
                    ...
                    <span className="text-[10px] text-indigo-400 text-center">画像を選択<br />またはドラッグ＆ドロップ</span>
                  )}
              ...
              <div className="flex flex-col gap-4 h-full">
                <div className="h-1/2 flex flex-col">
                  ...
                  <textarea className="w-full border border-indigo-200 rounded p-2 text-sm flex-1 resize-none bg-transparent focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-colors" value={data.userAspiration} onChange={(e) => onUpdate('userAspiration', e.target.value)} />
                </div>
                <div className="h-1/2 flex flex-col">
                  <label className="text-[10px] font-bold text-indigo-700 block mb-1 truncate">本人・家族の意向（家族）</label>
                  <textarea className="w-full border border-indigo-200 rounded p-2 text-sm flex-1 resize-none bg-transparent focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-colors" value={data.familyAspiration} onChange={(e) => onUpdate('familyAspiration', e.target.value)} />
                </div>
              </div>
            </div>
          </section>
      ...
      <div className="w-[210mm] h-[296mm] p-[10mm] relative box-border overflow-hidden" style={{ pageBreakAfter: 'always' }}>
        ...
        <div className="space-y-4">
          <section className="pdf-avoid-break">
            <SectionTitle title="3. 日常生活の自立判定・健康状態" pageNumber={2} />
            ...
            <div className="grid grid-cols-1 gap-4 mb-4">
              <div className="flex flex-col gap-1">
                ...
                <textarea className="border border-indigo-200 rounded p-2 text-sm h-16 resize-none bg-transparent focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-colors" value={data.medicalHistory} onChange={(e) => onUpdate('medicalHistory', e.target.value)} />
              </div>
            </div>
          ...
          <section className="pdf-avoid-break">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-8">
              <div className="flex flex-col h-full">
                <div className="flex justify-between items-center mb-2">
                  <label className="text-[10px] font-bold text-indigo-700 block truncate">間取り図（ドロップ可）</label>
                ...
                <div
                  className={`border-2 border-dashed rounded-xl flex-1 min-h-0 flex items-center justify-center relative overflow-hidden transition-all duration-200 cursor-pointer group
                    ${dragTarget === 'floor' ? 'border-indigo-500 bg-indigo-100 scale-[1.02] shadow-lg' : 'border-indigo-200 bg-indigo-50/50 hover:bg-indigo-50'}
                  `}
                ...
                >
                  {data.floorPlanUrl ? (
                    <>
                      <img src={data.floorPlanUrl} className="absolute inset-0 w-full h-full object-contain p-2" />
                      <button
                        onClick={(e) => handleRemoveImage('floorPlanUrl', e)}
                        className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity print:hidden"
                        title="画像を削除"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex flex-col gap-4 h-[240px]">
                <div className="h-1/2 flex flex-col">
                  <label className="text-[10px] font-bold text-indigo-700 block mb-1">今までの生活</label>
                  <textarea className="w-full border border-indigo-200 rounded p-2 text-sm flex-1 resize-none bg-transparent focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-colors" value={data.lifeHistory} onChange={(e) => onUpdate('lifeHistory', e.target.value)} />
                </div>
                <div className="h-1/2 flex flex-col">
                  <label className="text-[10px] font-bold text-indigo-700 block mb-1">現在の生活状況</label>
                  <textarea className="w-full border border-indigo-200 rounded p-2 text-sm flex-1 resize-none bg-transparent focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none transition-colors" value={data.currentSituation} onChange={(e) => onUpdate('currentSituation', e.target.value)} />
                </div>
              </div>
            </div>
          </section>
