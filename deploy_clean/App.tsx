...
const App: React.FC = () => {
  ...
  const handleFileSelect = (files: FileList | File[]) => {
    const validFiles = Array.from(files).filter(file => {
      ...
      // Allow Audio, PDF, Text, CSV
      return type.startsWith('audio/') || type === 'application/pdf' || type.startsWith('text/') || name.endsWith('.pdf') || name.endsWith('.txt') || name.endsWith('.csv');
    });
  ...
  const handleGenerateCarePlan = async () => {
    ...
    setError(null);
    try {
      const plan = await generateCarePlanFromAssessment(items, faceSheetData);
      setCarePlan(plan);
      setStatus(AppStatus.COMPLETED);
      setActiveTab('plan');
    } catch (err) {
